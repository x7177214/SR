...
import numpy as np
import scipy.io
from MODEL_div_l1_original_RELU import model # original : no gamma and beta ; with l1 regularization

#######Controler########
SCALE_FACTOR = 2
LAMBDA = 1.0 # total_loss = loss + LAMBDA * loss_lv + L1_regularization
TRAIN_DATA = 'LAPSR_manga'
########################
CHECKPOINT_PATH = "./checkpoints/x%d_div_l1_original_RELU_%.2flv_ON_%s" % (SCALE_FACTOR, LAMBDA, TRAIN_DATA)
...
if __name__ == '__main__':
    ...
    # predicted gradient map in x and y direction respectively
    ...
    # GT gradient map in x and y direction respectively
    ...
    # global var map
    var_map_output = tf.add(tf.abs(train_output_gx), tf.abs(train_output_gy))
    var_map_gt = tf.add(tf.abs(train_gt_gx), tf.abs(train_gt_gy))

    # local var map
    k_size = 3
    local_var_map_output = tf.nn.avg_pool(var_map_output, ksize=[1, k_size, k_size, 1], strides=[1, 1, 1, 1], padding='SAME')
    local_var_map_gt = tf.nn.avg_pool(var_map_gt, ksize=[1, k_size, k_size, 1], strides=[1, 1, 1, 1], padding='SAME')
    ...
    # [LOSS] local var loss in form of Chabonnier
    loss_lv = tf.reduce_sum(tf.sqrt(tf.square(local_var_map_output - local_var_map_gt)+tf.square(1e-3)))

    # [LOSS] l1 loss
    loss_v_l1 = 1 * tf.reduce_mean(loss_v_l1)

    
    loss = loss + LAMBDA * loss_lv + loss_v_l1
