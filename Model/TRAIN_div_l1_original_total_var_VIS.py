...
#######Controler########
SCALE_FACTOR = 2
LAMBDA = 0.5 # total_loss = loss + LAMBDA * loss_tv + L1_regularization
TRAIN_DATA = 'LAPSR_manga'
########################
CHECKPOINT_PATH = "./checkpoints/x%d_div_l1_original_%.2ftv_ON_" % (SCALE_FACTOR, LAMBDA) + TRAIN_DATA
TRAIN_DATA_PATH = "../dataset/mat/train/x%d/"%SCALE_FACTOR + TRAIN_DATA
...
if __name__ == '__main__':
    ...
    with tf.name_scope('loss'):
        ...
        tf.summary.scalar('loss_content', loss)
        
        # [LOSS] image total variation
        loss_tv = tf.reduce_sum(tf.image.total_variation(train_output))
        tf.summary.scalar('loss_tv * LAMBDA (= %.2f)' % LAMBDA, LAMBDA * loss_tv)
        ...
        if USE_QUEUE_LOADING:
            for epoch in xrange(START_EPOCH, MAX_EPOCH):
                for step in range(len(train_list) // BATCH_SIZE):
                    ...
                    summary, l1, _, l, output, lr, g_step = sess.run(
                        [merged, loss_v_l1, opt, loss, train_output, learning_rate, global_step])
                    print "[epoch %2.4f] loss %.4f l1 %.8f lr %.8f " % (epoch + (float(step) * BATCH_SIZE / len(train_list)), np.sum(l) / BATCH_SIZE, l1, lr)
                    
                    train_writer.add_summary(summary, step + (epoch * len(train_list)) // BATCH_SIZE)
