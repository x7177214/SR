...
def test_with_sess(epoch, ckpt_path, data_path, sess, shared_model):
    ...
    var_in = tf.placeholder(tf.float32, shape=(1, None, None, 1))
    var_out = local_var(var_in)
    ...
    for folder_path in folder_list:
        ...
        for i in range(len(img_list)):
            ...
            # y = np.array(y)
            # print(y.shape)
            # y = np.reshape(y, [1, 1, img_small.shape[0], img_small.shape[1], 1])
            ...
            img_raw = np.reshape(img_raw, [img_small.shape[0]*SCALE_FACTOR, img_small.shape[1]*SCALE_FACTOR, 1])
            gt_res = img_raw - img_3b
            gt_res = gt_res.reshape((1, img_small.shape[0]*SCALE_FACTOR, img_small.shape[1]*SCALE_FACTOR, 1))
            y_gt = sess.run([var_out], feed_dict={var_in: gt_res})
            y_gt = np.array(y_gt)


            y = y.reshape((1, img_small.shape[0]*SCALE_FACTOR, img_small.shape[1]*SCALE_FACTOR, 1))
            y = sess.run([var_out], feed_dict={var_in: y})
            y = np.array(y)
            ...
            METHOD = 'm4'
            y = np.reshape(y, [img_small.shape[0]*SCALE_FACTOR, img_small.shape[1]*SCALE_FACTOR, 1])
            y_gt = np.reshape(y_gt, [img_small.shape[0]*SCALE_FACTOR, img_small.shape[1]*SCALE_FACTOR, 1])
 
            print(y.shape)

            result = np.concatenate((y, y, y), axis=2)
            result[np.where(result < 0.0)] = 0.0
            result[np.where(result > 255.0)] = 255.0
            result = Image.fromarray(np.uint8(result), mode='RGB')
                       
            result2 = np.concatenate((y_gt, y_gt, y_gt), axis=2)
            result2[np.where(result2 < 0.0)] = 0.0
            result2[np.where(result2 > 255.0)] = 255.0
            result2 = Image.fromarray(np.uint8(result2), mode='RGB')

            result3 = np.concatenate((y_255, y_255, y_255), axis=2)
            result3[np.where(result3 < 0.0)] = 0.0
            result3[np.where(result3 > 255.0)] = 255.0
            result3 = Image.fromarray(np.uint8(result3), mode='RGB')

            #Save the result image
            if SAVEIMAGE_OR_DISPLAYPSNR == 0:
                if not os.path.exists(SAVE_PATH +'_'+ DATASET):
                    os.makedirs(SAVE_PATH +'_'+ DATASET)
                result.save(SAVE_PATH +'_'+ DATASET + '/%d_%d_%s_res_var.bmp' % (i, epoch, METHOD))
                result2.save(SAVE_PATH +'_'+ DATASET + '/%d_%d_%s_gt_var.bmp' % (i, epoch, METHOD))
                result3.save(SAVE_PATH +'_'+ DATASET + '/%d_%d_%s_res.bmp' % (i, epoch, METHOD))
