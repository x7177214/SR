...
def save_images(imgs, layer):
    path = SAVE_PATH +'_'+ DATASET+'/'+layer
    if not os.path.exists(path):
        os.makedirs(path)

    imgs = np.array(imgs, dtype=np.float32)
    imgs = imgs * 255.0
    imgs[np.where(imgs < 0.0)] = 0.0
    imgs[np.where(imgs > 255.0)] = 255.0
...
def test_with_sess(epoch, ckpt_path, data_path, sess, shared_model):
    ...
    for folder_path in folder_list:
        ...
        for i in range(len(img_list)):
            ...
            # y, fm = sess.run([output_tensor, feature_map], feed_dict={input_tensor: y})
            ...
            y_res = y_res[:, :, 0]
            y_res = y_res*255.0
            y_res[np.where(y_res < 0.0)] = 0.0
            y_res[np.where(y_res > 255.0)] = 255.0
            result2 = Image.fromarray(np.uint8(y_res), mode='L')

            img_3b = img_3b[:, :, 0]
            img_3b = img_3b*255.0
            img_3b[np.where(img_3b < 0.0)] = 0.0
            img_3b[np.where(img_3b > 255.0)] = 255.0
            ...
            if SAVEIMAGE_OR_DISPLAYPSNR == 0:
                ...
                result2.save(SAVE_PATH +'_'+ DATASET + '/%d_%d_%s_res.bmp' % (i, epoch, METHOD))
                result3.save(SAVE_PATH +'_'+ DATASET + '/%d_%d_%s_bic.bmp' % (i, epoch, METHOD))
