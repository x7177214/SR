...
def model(input_tensor, bic_tesor, r, train):
    ...
    # Conv5 3x3@1
    conv_5_w = tf.get_variable("conv_5_w", [3, 3, 32, 1],
                               initializer=tf.random_normal_initializer(stddev=0.001))
    conv_5_b = tf.get_variable("conv_5_b", [1],
                               initializer=tf.constant_initializer(0))
    ...
    # global avg. pool
    dim = tensor.get_shape().as_list()
    tensor = tf.nn.avg_pool(tensor, ksize=[1, dim[1], dim[2], 1],
                                strides=[1, 1, 1, 1], padding='VALID')
        # tensor = tf.reduce_mean(tensor, 1) # [N, H, W , C] -> [N, W , C]
        # tensor = tf.reduce_mean(tensor, 1) # [N, W , C] -> [N, C]
