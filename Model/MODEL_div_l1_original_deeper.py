...
def div_norm(input_, weights, loss_v_l1, name="div_norm", k_size=3):
    with tf.variable_scope(name):
        ...
        loss_v_l1.append(tf.reduce_mean(tf.sqrt(tf.cast(v**2 + (1e-3)**2, tf.float32)))) # to use sqrt, float32 is needed
...
def model(input_tensor, bic_tesor, r, train):
    ...
    conv_1_w = tf.get_variable("conv_1_w", [5, 5, 1, 32],
                               initializer=tf.random_normal_initializer(stddev=0.001), dtype=tf.float32)
    ...
    # Conv2 1x1@8
    conv_2_w = tf.get_variable("conv_2_w", [1, 1, 32, 8],
                               initializer=tf.random_normal_initializer(stddev=0.001), dtype=tf.float32)
    conv_2_b = tf.get_variable("conv_2_b", [8],
                               initializer=tf.constant_initializer(0), dtype=tf.float32)
    ...
    # Conv3 3x3@8
    conv_3_w = tf.get_variable("conv_3_w", [3, 3, 8, 8],
                               initializer=tf.random_normal_initializer(stddev=0.001), dtype=tf.float32)
    conv_3_b = tf.get_variable("conv_3_b", [8],
                               initializer=tf.constant_initializer(0), dtype=tf.float32)
    ...
    # Conv4 3x3@8
    conv_4_w = tf.get_variable("conv_4_w", [3, 3, 8, 8],
                               initializer=tf.random_normal_initializer(stddev=0.001), dtype=tf.float32)
    conv_4_b = tf.get_variable("conv_4_b", [8],
                               initializer=tf.constant_initializer(0), dtype=tf.float32)
    ...
    # Conv5 3x3@8
    conv_5_w = tf.get_variable("conv_5_w", [3, 3, 8, 8],
                               initializer=tf.random_normal_initializer(stddev=0.001), dtype=tf.float32)
    conv_5_b = tf.get_variable("conv_5_b", [8],
                               initializer=tf.constant_initializer(0), dtype=tf.float32)
    ...
    tensor, weights, loss_v_l1 = div_norm(tensor, weights, loss_v_l1, name='norm_5')
    tensor, weights = prelu(tensor, weights, scope='prelu_5')

    # Conv6 1x1@32
    conv_6_w = tf.get_variable("conv_6_w", [1, 1, 8, 32],
                               initializer=tf.random_normal_initializer(stddev=0.001), dtype=tf.float32)
    conv_6_b = tf.get_variable("conv_6_b", [32],
                               initializer=tf.constant_initializer(0), dtype=tf.float32)
    weights.append(conv_6_w)
    weights.append(conv_6_b)
    tensor = tf.nn.bias_add(tf.nn.conv2d(tensor, conv_6_w,
                                         strides=[1, 1, 1, 1], padding='SAME'), conv_6_b)
    tensor, weights, loss_v_l1 = div_norm(tensor, weights, loss_v_l1, name='norm_6')
    tensor, weights = prelu(tensor, weights, scope='prelu_6')

    # Conv7 3x3@r^2
    conv_7_w = tf.get_variable("conv_7_w", [3, 3, 32, r * r],
                               initializer=tf.random_normal_initializer(stddev=0.001), dtype=tf.float32)
    conv_7_b = tf.get_variable("conv_7_b", [r * r],
                               initializer=tf.constant_initializer(0), dtype=tf.float32)
    weights.append(conv_7_w)
    weights.append(conv_7_b)
    tensor = tf.nn.bias_add(tf.nn.conv2d(tensor, conv_7_w,
                                         strides=[1, 1, 1, 1], padding='SAME'), conv_7_b)
    if train:
        # Sub-pixel layer
        tensor = sb(tensor, r)
