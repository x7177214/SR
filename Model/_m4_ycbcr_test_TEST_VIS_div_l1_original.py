...
# from ycbcr2rgb import ycbcr2rgb

SCALE_FACTOR = 2
DATASET = "nova_d" # testForValidation2_d
...
def test_with_sess(epoch, ckpt_path, data_path, sess, shared_model):
    ...
    for folder_path in folder_list:
        ...
        for i in range(len(img_list)):
            ...
            '''
            # method_3: the same formula used in method 1, but implemented by hand and the truncation is added
            METHOD = 'm3'
            ...
            # truncation
            # y[np.where(y <= 16.0)] = 0.0
            # y[np.where(y >= 235.0)] = 255.0

            # cb[np.where(cb <= 16.0)] = 0.5
            # cb[np.where(cb >= 235.0)] = 255.5

            # cr[np.where(cr <= 16.0)] = 0.5
            # cr[np.where(cr >= 235.0)] = 255.5  
            ...
            # 0~255
            new_Yres = 0.2989 * result[:, :, 0] + 0.5870 * result[:, :, 1] + 0.1140 * result[:, :, 2]
            
            new_Yres = np.reshape(new_Yres, (img_small.shape[0]*SCALE_FACTOR, img_small.shape[1]*SCALE_FACTOR, 1))
            
            y = new_Yres + img_3b * 255.0
            ...
            if SAVEIMAGE_OR_DISPLAYPSNR == 0:
                ...
                result.save(SAVE_PATH +'_'+ DATASET + '/%d_%d_%s2.bmp' % (i, epoch, METHOD))
