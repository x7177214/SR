...
def model(input_tensor, bic_tesor, r, train):
    ...
    # Conv3-1 1x1@5
    conv_3_1_w = tf.get_variable("conv_3_1_w", [1, 1, 5, 5],
                               initializer=tf.random_normal_initializer(stddev=0.001))
    conv_3_1_b = tf.get_variable("conv_3_1_b", [5],
                               initializer=tf.constant_initializer(0))
    weights.append(conv_3_1_w)
    weights.append(conv_3_1_b)
    tensor_3_1 = tf.nn.bias_add(tf.nn.conv2d(tensor, conv_3_1_w,
                                         strides=[1, 1, 1, 1], padding='SAME'), conv_3_1_b)
    tensor_3_1, weights, loss_v_l1 = div_norm(tensor_3_1, weights, loss_v_l1, name='norm_3_1')
    tensor_3_1, weights = prelu(tensor_3_1, weights, scope='prelu_3_1')

    # Conv3-2 3x3@5
    conv_3_2_w = tf.get_variable("conv_3_2_w", [3, 3, 5, 5],
                               initializer=tf.random_normal_initializer(stddev=0.001))
    conv_3_2_b = tf.get_variable("conv_3_2_b", [5],
                               initializer=tf.constant_initializer(0))
    weights.append(conv_3_2_w)
    weights.append(conv_3_2_b)
    tensor_3_2 = tf.nn.bias_add(tf.nn.conv2d(tensor, conv_3_2_w,
                                         strides=[1, 1, 1, 1], padding='SAME'), conv_3_2_b)
    tensor_3_2, weights, loss_v_l1 = div_norm(tensor_3_2, weights, loss_v_l1, name='norm_3_2')
    tensor_3_2, weights = prelu(tensor_3_2, weights, scope='prelu_3_2')

    # inception additon
    tensor = tf.add(tensor_3_1, tensor_3_2)
