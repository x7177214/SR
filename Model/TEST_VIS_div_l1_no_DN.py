...
from MODEL_div_l1_no_DN import model
from Subpixel import sb_test

SCALE_FACTOR = 4
DATASET = "LAPCG_d"
EPOCH = 500 # Epoch you want to test
SAVEIMAGE_OR_DISPLAYPSNR = 0
'''
0: Save Image, 1: Display
'''
#DATA_PATH = "../data/test/x%d_div_l1_gamma_beta_plus_manga/"%SCALE_FACTOR
DATA_PATH = "../dataset/mat/test/x%d/"%SCALE_FACTOR
CHECKPOINTS_PATH = "./checkpoints/x%d_div_l1_gamma_beta_plus_manga"%SCALE_FACTOR
IMAGE_FORMAT = "*.bmp" #*.jpg or *.bmp
SAVE_PATH = "./result/x%d_div_l1_gamma_beta_plus_manga"%SCALE_FACTOR
...
def test_with_sess(epoch, ckpt_path, data_path, sess, shared_model):
    ...
    for folder_path in folder_list:
        ...
        for i in range(len(img_list)):
            ...
            y = (y + img_3b) * 255
            y[np.where(y < 0)] = 0
            y[np.where(y > 255)] = 255
            img_cbcr = img_cbcr * 255
            ...
            if SAVEIMAGE_OR_DISPLAYPSNR == 0:
                ...
                result.save(SAVE_PATH +'_'+ DATASET + '/%d_%d.bmp' % (i, epoch))
