...
#######Controler########
SCALE_FACTOR = 2
LAMBDA = 0.9 # total_loss = (1 - LAMBDA) * CHAR_at_non_edge + LAMBDA * CHAR_at_edge + L1_regularization
TRAIN_DATA = 'LAPSR_more_manga_edge' # LAPSR_more_manga_edge Set5_edge
########################
CHECKPOINT_PATH = "./checkpoints/x%d_div_l1_original_%.2fedge_ON_%s" % (SCALE_FACTOR, LAMBDA, TRAIN_DATA) 
    ...
    for f in l:
        if os.path.exists(f):
            if os.path.exists(f[:-4] + "_2.mat"):
                train_list.append([f, f[:-4] + "_2.mat", f[:-4] + "_2b.mat", f[:-4] + "_2edge.mat"])
            if os.path.exists(f[:-4] + "_3.mat"):
                train_list.append([f, f[:-4] + "_3.mat", f[:-4] + "_3b.mat", f[:-4] + "_3edge.mat"])
            if os.path.exists(f[:-4] + "_4.mat"):
                train_list.append([f, f[:-4] + "_4.mat", f[:-4] + "_4b.mat", f[:-4] + "_4edge.mat"])
    return train_list
...
def get_image_batch(train_list, offset, batch_size):
    ...
    for pair in target_list:
        ...
        edge = scipy.io.loadmat(pair[3])['edge_patch'] 
    ...
    edge_list.resize([BATCH_SIZE, OUT_IMG_SIZE[1], OUT_IMG_SIZE[0], 1])

    return input_list, gt_list, edge_list
...
if __name__ == '__main__':
    ...
    if not USE_QUEUE_LOADING:
        ...
        train_gt_single = tf.placeholder(
            tf.float32, shape=(OUT_IMG_SIZE[0], OUT_IMG_SIZE[1], 1))
        train_edge_single = tf.placeholder(
            tf.float32, shape=(OUT_IMG_SIZE[0], OUT_IMG_SIZE[1], 1))

        q = tf.FIFOQueue(1000, [tf.float32, tf.float32, tf.float32, tf.float32], [
                         [OUT_IMG_SIZE[0], OUT_IMG_SIZE[1], 1],
                         [IN_IMG_SIZE[0], IN_IMG_SIZE[1], 1],
                         [OUT_IMG_SIZE[0], OUT_IMG_SIZE[1], 1],
                         [OUT_IMG_SIZE[0], OUT_IMG_SIZE[1], 1]]
                         )
        enqueue_op = q.enqueue([train_bic_single, train_input_single, train_gt_single, train_edge_single])
        train_bic, train_input, train_gt, train_gt_edge = q.dequeue_many(BATCH_SIZE)
    ...
    # [LOSS] Chabonnier loss modulated by edge map
    train_gt_edge = tf.multiply(train_gt_edge, 2.0 * LAMBDA - 1.0) # modulate the edge map to serve as the weights in loss calculation
    loss = tf.reduce_sum(tf.multiply(tf.sqrt(tf.square(train_output - train_gt)+tf.square(1e-3)), train_gt_edge + 1.0 - LAMBDA))

    # loss = tf.reduce_sum(tf.multiply(tf.sqrt(tf.square(train_output - train_gt)+tf.square(1e-3)), train_gt_edge))
        ...
        if USE_QUEUE_LOADING:
            def load_and_enqueue(coord, file_list, idx=0, num_thread=1):
                ...
                while not coord.should_stop():
                    ...
                    # modulate the edge map to serve as the weights in loss calculation
                    # gt_edge_img[np.where(gt_edge_img == 0.0)] = 1.0 - LAMBDA
                    # gt_edge_img[np.where(gt_edge_img == 1.0)] = LAMBDA
                    # print gt_edge_img.shape
                    # gt_edge_img[np.where(gt_edge_img == 0.0)] = 1.0
                    # gt_edge_img[np.where(gt_edge_img == 1.0)] = 0.0

                    sess.run(enqueue_op, feed_dict={train_bic_single: bic_img,
                            train_input_single: input_img, 
                            train_gt_single: gt_img,
                            train_edge_single: gt_edge_img
                            })
                    count += 1
        ...
        if USE_QUEUE_LOADING:
            ...
            for epoch in xrange(START_EPOCH, MAX_EPOCH):
                for step in range(len(train_list) // BATCH_SIZE):
                    ...
                    input_data, gt_data, gt_edge_data = get_image_batch(
                    ...
                    del input_data, gt_data, gt_edge_data
                if epoch % 10 == 0:
                    saver.save(sess, CHECKPOINT_PATH + "/epoch_%03d.ckpt" %
                               epoch, global_step=global_step)
