...
import numpy as np
import scipy.io
from MODEL_div_l1_original_two_sub import model # original : no gamma and beta ; with l1 regularization
#######Controler########
SCALE_FACTOR = 2
LAMBDA = 0.25 # total_loss = loss + LAMBDA * loss_tv + L1_regularization
TRAIN_DATA = 'LAPSR_manga'
########################
CHECKPOINT_PATH = "./checkpoints/x%d_div_l1_original_two_sub_%.2ftv_ON_%s" % (SCALE_FACTOR, LAMBDA, TRAIN_DATA)
            ...
            def load_and_enqueue(coord, file_list, idx=0, num_thread=1):
                ...
                while not coord.should_stop():
                    ...
                    gt_img = scipy.io.loadmat(file_list[i][0])['patch'].reshape(
                        [OUT_IMG_SIZE[0], OUT_IMG_SIZE[1], 1])
                    sess.run(enqueue_op, feed_dict={train_input_single: input_img, 
        ...
        if USE_QUEUE_LOADING:
            for epoch in xrange(START_EPOCH, MAX_EPOCH):
                for step in range(len(train_list) // BATCH_SIZE):
                    ...
                    print "[epoch %2.4f] loss %.4f LAMBDA*ltv %.2f lr %.8f " % (epoch + (float(step) * BATCH_SIZE / len(train_list)), np.sum(l) / BATCH_SIZE, LAMBDA*np.sum(ltv) / BATCH_SIZE, lr)
