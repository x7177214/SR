...
% of these image patch pairs (LR, groundtruth, GT edge map, restored image)

% restored image :
%     gt image -> bicubic small image -> bicubic original size image

% GT edge map :
%     gt image -> (edge_detection.m) -> strong edge map
...
save_path = '../train/x2/LAPSR_manga_edge_blur/'; % a '/' must be added at end of the path
...
for f_iter = 1:numel(f_lst)
    ...
    % (original size)
                ...
                patch2 = patch;
    % (GT edge map)
                patch_name2 = sprintf('%s%d%s', save_path, count, ['_', num2str(scale_factor), 'edge']); 
                edge_patch = edge_detection(patch);
